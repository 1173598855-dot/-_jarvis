...
class FileCapabilityStore:
    ...
    def _verify_archive(
        ...
        entries = self._bounded_archive_entries(archive)

        names = []
        seen_names = set()
        logical_entries: dict[str, set[bool]] = {}
        total_declared = 0
        for info in entries:
            ...
            logical_path = name.rstrip("/").casefold()
            logical_entries.setdefault(logical_path, set()).add(info.is_dir())
        ...
        for path, kinds in logical_entries.items():
            if len(kinds) != 1:
                raise CapabilityStoreError(
                    "ARCHIVE_ENTRY_CONFLICT",
                    "archive entries have conflicting types",
                )
            is_directory = next(iter(kinds))
            for other_path, other_kinds in logical_entries.items():
                if path == other_path:
                    continue
                other_is_directory = next(iter(other_kinds))
                if not is_directory and other_path.startswith(path + "/"):
                    raise CapabilityStoreError(
                        "ARCHIVE_ENTRY_CONFLICT",
                        "archive entries have conflicting paths",
                    )
                if not other_is_directory and path.startswith(other_path + "/"):
                    raise CapabilityStoreError(
                        "ARCHIVE_ENTRY_CONFLICT",
                        "archive entries have conflicting paths",
                    )
    ...
    def _bounded_archive_entries(self, archive: zipfile.ZipFile) -> list[zipfile.ZipInfo]:
        try:
            entries = archive.filelist
        except AttributeError as exc:
            raise CapabilityStoreError("ARCHIVE_INVALID", "archive entries cannot be read") from exc
        if not isinstance(entries, list):
            raise CapabilityStoreError("ARCHIVE_INVALID", "archive entries cannot be read")
    ...
    def _publish_revision(self, bundle: bytes, installed: InstalledCapability) -> None:
        ...
        try:
            ...
            with zipfile.ZipFile(io.BytesIO(bundle), "r") as archive:
                for info in self._bounded_archive_entries(archive):
                    name = _validate_member_name(info)
    ...
    def _verify_published_content(
        ...
        observed_entries = 1
        pending_directories = [payload_path]
        while pending_directories:
            current_path = pending_directories.pop()
            self._assert_store_path(current_path)
            with os.scandir(current_path) as entries:
                for entry in entries:
                    ...
                    relative_name = child_path.relative_to(revision_path).as_posix()
