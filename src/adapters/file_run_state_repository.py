...
from core.contracts.run_state import SCHEMA_VERSION, RunState
...
_MAX_RECOVERY_FILE_BYTES = 8 * 1024 * 1024
_MAX_RECOVERY_TOTAL_BYTES = 32 * 1024 * 1024
_MAX_RECOVERY_DIRECTORY_ENTRIES = 8_192
...
def _fsync_directory(directory: Path) -> None:
    """Flush one directory entry, raising this repository's integrity error.

    The platform, no-follow and unsupported-filesystem rules live in the shared
    `durable_directory` contract; only the error type is repository-specific.
    """
    try:
        fsync_directory(directory)
    except OSError as exc:
        raise RunStateIntegrityError(
            f"cannot flush the {directory.name} directory entry"
        ) from exc
...
def _read_bounded_bytes(path: Path, *, label: str) -> bytes:
    try:
        if path.stat().st_size > _MAX_RECOVERY_FILE_BYTES:
            raise RunStateIntegrityError(f"{label} exceeds recovery file size limit")
        with path.open("rb") as handle:
            content = handle.read(_MAX_RECOVERY_FILE_BYTES + 1)
    except RunStateIntegrityError:
        raise
    except OSError as exc:
        raise RunStateIntegrityError(f"{label} is unreadable") from exc
    if len(content) > _MAX_RECOVERY_FILE_BYTES:
        raise RunStateIntegrityError(f"{label} exceeds recovery file size limit")
    return content
...
class FileRunStateRepository:
    ...
    def _save_locked(
        ...
        if existing is None and run_directory.exists():
            run_children = self._bounded_directory_children(run_directory, 2)
            if run_children and not self._can_resume_unpublished_revision(
                run_directory,
                sanitized_state.revision,
            ):
                raise ValueError(
                    "run_id already has archived or incomplete recovery data"
                )
    ...
    def _load_authenticated_active_locked(self) -> _AuthenticatedActive | None:
        ...
        for name in _FILE_NAMES:
            ...
            content = _read_bounded_bytes(
                path,
                label=f"recovery file for {name}",
            )
            total_loaded += len(content)
            if total_loaded > _MAX_RECOVERY_TOTAL_BYTES:
                raise RunStateIntegrityError("recovery snapshot exceeds size limit")
    ...
    def archive_active(self, expected_run_id: str) -> None:
        with self._exclusive_write():
            ...
            if archive_path.exists() or _is_reparse_point(archive_path):
                ...
                try:
                    existing_archive = _read_bounded_bytes(
                        archive_path,
                        label="archived run manifest",
                    )
                except OSError as exc:
                    raise RunStateIntegrityError(
    ...
    @staticmethod
    def _bounded_directory_children(
        directory: Path,
        limit: int,
    ) -> tuple[Path, ...]:
        if type(limit) is not int or limit < 0:
            raise ValueError("directory entry limit must be a non-negative integer")
        children: list[Path] = []
        try:
    ...
    @staticmethod
    def _bounded_fd_directory_names(
        directory_fd: int,
        limit: int,
    ) -> tuple[str, ...]:
        if type(limit) is not int or limit < 0:
        ...
        try:
            ...
            raise RunStateIntegrityError(
                "cleanup directory is unreadable"
            ) from exc
        return tuple(names)
    ...
    def _can_resume_unpublished_revision(
        ...
        try:
            run_children = FileRunStateRepository._bounded_directory_children(
                run_directory,
                1,
            )
            revision_children = FileRunStateRepository._bounded_directory_children(
                revisions,
                _MAX_RECOVERY_DIRECTORY_ENTRIES,
            )
        except (FileNotFoundError, OSError, RunStateIntegrityError):
            return False
        if (
    ...
    def _prune_revision_snapshots(self, run_id: str, keep_revision: int) -> None:
        ...
        try:
            with self._open_repository_directory(
                ...
                candidates = self._bounded_fd_directory_names(
                    revisions_fd,
                    _MAX_RECOVERY_DIRECTORY_ENTRIES,
                )
        ...
        except (OSError, RunStateIntegrityError):
            return
    ...
    def _discard_flat_directory(parent_fd: int, directory_name: str) -> None:
        ...
        try:
            ...
            return
        try:
            children = FileRunStateRepository._bounded_fd_directory_names(
                directory_fd,
                len(_FILE_NAMES),
            )
    ...
    def _atomic_write(self, path: Path, content: bytes) -> None:
        if len(content) > _MAX_RECOVERY_FILE_BYTES:
            raise RunStateIntegrityError(
                f"{path.name} exceeds recovery file size limit"
            )
        ...
        try:
            ...
            actual_content = _read_bounded_bytes(
                path,
                label=f"{path.name} read-back",
            )
    ...
    def _publish_revision(
        ...
        try:
            ...
            published = True
            _fsync_directory(revision_directory.parent)
            self._verify_revision_contents(revision_directory, contents)
        finally:
            if not published:
    ...
    def _verify_revision_contents(
        ...
        children = self._bounded_directory_children(
            revision_directory,
            len(_FILE_NAMES),
        )
    ...
    def _read_json(self, path: Path, field_name: str) -> object:
        try:
            return json.loads(
                _read_bounded_bytes(path, label=field_name).decode("utf-8")
            )
