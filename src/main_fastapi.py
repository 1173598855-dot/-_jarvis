...
from core.kernel.plugin_broker import FIRST_PARTY_EVENT_GRANTS
from core.kernel.plugin_sdk import PLUGIN_API_VERSION, PluginManager
...
from core.kernel.terminal_executor import TerminalCommand
...
def _raise_query_too_large() -> None:
    raise HTTPException(
        status_code=413,
        detail={
            "code": "REQUEST_QUERY_TOO_LARGE",
            "message": REQUEST_QUERY_TOO_LARGE_MESSAGE,
        },
    )


class _RequestQueryLimitMiddleware:
    """Reject oversized API query strings before framework parsing."""

    def __init__(self, app, max_query_bytes: int = MAX_REQUEST_QUERY_BYTES):
        self.app = app
    ...
    async def __call__(self, scope, receive, send):
        if (
            ...
            and len(scope.get("query_string", b"")) > self.max_query_bytes
        ):
            await JSONResponse(
                status_code=413,
                content={
...
app.add_middleware(_RequestQueryLimitMiddleware)
app.add_middleware(
    CORSMiddleware,
...
async def request_validation_exception_handler(
    ...
    is_stream_query_too_large = request.url.path == "/api/ollama/chat/stream" and any(
        error.get("type") in {"string_too_long", "too_long"}
        and tuple(error.get("loc", ()))[:1] == ("query",)
        and tuple(error.get("loc", ()))[1:] in {("model",), ("messages",)}
        for error in errors
    )
...
async def ollama_chat_stream(
    model: str = Query(default="default", max_length=MAX_REQUEST_QUERY_BYTES),
    messages: str = Query(default="[]", max_length=MAX_REQUEST_QUERY_BYTES),
):
    """Streaming chat (SSE) with query parameters for EventSource clients."""
    if not _query_value_within_limit(model) or not _query_value_within_limit(messages):
        _raise_query_too_large()
    try:
...
async def events(limit: int = 100):
    """Return the bounded history owned by this FastAPI service."""
...
def create_app(app_state: Optional[AppState] = None) -> FastAPI:
    ...
    application.add_middleware(_RequestQueryLimitMiddleware)
