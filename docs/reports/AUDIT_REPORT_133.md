...
- **Orphan reconciliation** (RoleWorkerSupervisor.recover_orphans()):
  ...
  - Duplicate task_id values are silently skipped.
...
- **Startup wiring** (AppState in src/main_fastapi.py):
  ...
  - The FastAPI lifespan loads persisted records after run_lifecycle.recover_active(),
    reconciles orphans via recover_orphans(), then clears the persisted file.
...
- src/core/brain/role_worker.py — added recover_orphans()
