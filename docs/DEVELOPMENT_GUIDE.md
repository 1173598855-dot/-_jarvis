...
状态：进行中。Iteration 135-136 已交付 D1-D2：只读发现器可为仓库内 Skill、Plugin 和直接导出的 UI 组件生成版本化记录、相对路径、确定性摘要、来源完整性、健康和风险状态；严格的数字版本约束和确定性本地解析可按类型、风险与兼容状态过滤。扫描和解析不导入或执行能力代码。安全包存储、可逆生命周期与 API/UI 接入尚未完成。
