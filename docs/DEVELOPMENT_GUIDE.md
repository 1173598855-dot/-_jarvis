...
状态：已完成。Iteration 135-139 已交付版本化只读发现、严格兼容求值和确定性解析、内容寻址的禁用包验证与暂存、原子升级/回滚/删除/重建，以及 OpenAPI `1.14.0` 三服务只读接口和 Plugins 视图；Iteration 140 将共享契约升级到 `1.15.0` 并加入 Express-only Git 路径边界。发现、解析、存储和 HTTP/UI 展示均不导入或执行能力代码；HTTP 不接受归档、路径、URL 或生命周期写操作。
