...
class TestFileCapabilityStore(unittest.TestCase):
    ...
    def test_install_does_not_copy_archive_entries_through_infolist(self):
        with patch.object(
            zipfile.ZipFile,
            "infolist",
            side_effect=AssertionError("unbounded archive entry copy"),
        ):
            installed = self.store.install(self.bundle, self.digest)

        self.assertEqual(installed.capability_id, "skill:package-example")
        self.assertEqual(installed.revision_id, self.digest)
    ...
    def test_rebuild_stops_scandir_at_first_published_entry_over_limit(self):
        root = self.temp_root / "bounded-tree"
        store = FileCapabilityStore(root, CapabilityPackageLimits(max_files=2))
        installed = store.install(self.bundle, self.digest)
        payload_path = root / Path(installed.relative_path) / "payload"
        for index in range(3):
            (payload_path / f"zz-{index}").mkdir()
        ...
        class LimitedScandir:
            ...
            def __next__(self):
                ...
                calls += 1
                if calls > 2:
                    raise AssertionError("published tree was enumerated past its limit")
                return next(self._entries)

        with patch("adapters.file_capability_store.os.scandir", LimitedScandir):
            with self.assertRaises(CapabilityStoreError) as raised:
                store.rebuild(installed.capability_id)
