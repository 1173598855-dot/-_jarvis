...
from core.brain.role_tools import RoleToolDeniedError
...
class TestReadOnlyRoleTools(unittest.TestCase):
    ...
    def test_broker_rejects_missing_or_untyped_assembly(self):
        with tempfile.TemporaryDirectory() as root:
            for assembly in (None, object()):
                with self.subTest(assembly=assembly):
                    with self.assertRaises(RoleToolProtocolError):
                        create_read_only_role_tool_broker(
                            self.manager,
    ...
    def test_memory_search_returns_clean_v2_body_without_storage_footer(self):
        ...
        self.assertEqual(found["results"][0]["snippet"], entry.content)
        self.assertNotIn("**metadata**", str(found))
        self.assertNotIn("**tags**", str(found))

    def test_memory_search_ranks_matches_and_returns_bounded_provenance(self):
        with tempfile.TemporaryDirectory() as root:
            memory_dir = Path(root) / "memory"
            store = MemoryStore(memory_dir=str(memory_dir))
            strong = MemoryEntry.create(
                MemoryType.PROJECT,
                "Deployment config",
                "The blue deployment config is selected for release.",
                metadata={
                    "source_id": "source-alpha",
                    "merged_from": ["merge-a", "merge-b", 7],
                },
                tags=["deployment"],
            )
            weak = MemoryEntry.create(
                MemoryType.REFERENCE,
                "Later note",
                "A brief deployment mention.",
            )
            store.store(strong)
            store.store(weak)
            broker = self.make_broker(memory_dir, root)
        ...
        self.assertEqual(
            [item["id"] for item in found["results"]],
            [strong.id, weak.id],
        )
        self.assertGreater(found["results"][0]["score"], found["results"][1]["score"])
        self.assertEqual(found["results"][0]["parent_id"], strong.parent_id)
        self.assertEqual(
            found["results"][0]["source_ids"],
            ["source-alpha", "merge-a", "merge-b"],
        )
        self.assertNotIn("metadata", str(found))

    def test_memory_search_centers_the_snippet_on_the_first_match(self):
        with tempfile.TemporaryDirectory() as root:
            memory_dir = Path(root) / "memory"
            store = MemoryStore(memory_dir=str(memory_dir))
            prefix = "lead-" + ("context " * 120)
            entry = MemoryEntry.create(
                MemoryType.PROJECT,
                "Needle",
                prefix + "NEEDLE tail text",
            )
            store.store(entry)
            broker = self.make_broker(memory_dir, root)
        ...
        self.assertLessEqual(len(snippet), 512)
        self.assertIn("NEEDLE", snippet)
        self.assertNotIn("lead-context", snippet)
