...
from core.kernel.event_bus import Event, EventBus  # noqa: E402
...
class TestMainHTTPGETRouting(unittest.TestCase):
    ...
    def test_do_get_accepts_exact_raw_query_budget(self):
        handler = self._make_routed_handler(
            "/api/health?" + "x" * _main.MAX_REQUEST_QUERY_BYTES
        )
        handler.handle_health = MagicMock()
    ...
    def test_api_routes_reject_raw_query_over_budget_before_dispatch(self):
        oversized_query = "x" * (_main.MAX_REQUEST_QUERY_BYTES + 1)
        cases = (
            ("GET", "/api/health", "do_GET", "handle_health"),
            (
                "POST",
                "/api/ollama/token-usage",
                "do_POST",
                "handle_ollama_token_usage",
            ),
            (
                "DELETE",
                "/api/memory/probes/user/fixture",
                "do_DELETE",
                "handle_memory_delete",
            ),
        )
        for method, path, dispatch_name, handler_name in cases:
            with self.subTest(method=method):
                handler = self._make_routed_handler(
                    f"{path}?{oversized_query}",
                    command=method,
                )
                handler._send_error = MagicMock()
                setattr(handler, handler_name, MagicMock())

                getattr(handler, dispatch_name)()
...
class TestMainHTTPHandleMethodsRouting(unittest.TestCase):
    ...
    def test_handle_ollama_stream_rejects_oversized_raw_query_before_parsing(self):
        query = "model=" + ("m" * _main.MAX_REQUEST_BODY_BYTES)
        handler = _make_handler(
            path=f"/api/ollama/chat/stream?{query}"
        )
        handler._send_error = MagicMock()
        handler._stream_ollama_chat = MagicMock()

        with patch.object(
            _main,
            "parse_qs",
            return_value={"model": ["default"], "messages": ["[]"]},
        ) as parse_query:
            handler.handle_ollama_chat_stream()

        parse_query.assert_not_called()
        handler._stream_ollama_chat.assert_not_called()
        handler._send_error.assert_called_once_with(
            "Request query exceeds the 32 KiB limit",
            413,
            "REQUEST_QUERY_TOO_LARGE",
    ...
    def test_handle_ollama_stream_rejects_oversized_decoded_value(self):
        handler = _make_handler(
            path="/api/ollama/chat/stream?messages=%5B%5D"
        )
        handler._send_error = MagicMock()
        handler._stream_ollama_chat = MagicMock()
        oversized_messages = "x" * (_main.MAX_REQUEST_BODY_BYTES + 1)

        with patch.object(
            _main,
            "parse_qs",
            return_value={"model": ["default"], "messages": [oversized_messages]},
        ):
            handler.handle_ollama_chat_stream()
    ...
    def test_handle_ollama_stream_accepts_exact_raw_query_budget(self):
        model = "m" * (_main.MAX_REQUEST_BODY_BYTES - len("model="))
        query = f"model={model}"
        self.assertEqual(len(query.encode("utf-8")), _main.MAX_REQUEST_BODY_BYTES)
        handler = _make_handler(
            path=f"/api/ollama/chat/stream?{query}"
        )
        handler._stream_ollama_chat = MagicMock()
        ...
        handler._stream_ollama_chat.assert_called_once_with(model, [])
    ...
    def test_handle_plugins_list_uses_its_app_state_manager(self):
        manager = MagicMock()
    ...
    def test_handle_memory_entries_preserves_v2_fields_and_clean_content(self):
        import tempfile

        with tempfile.TemporaryDirectory() as tmp:
            memory_store = _main.MemoryStore(memory_dir=tmp)
            entry = _main.MemoryEntry.create(
                _main.MemoryType.PROJECT,
                "stdlib round trip",
                "clean stdlib body",
                tags=["api", "stdlib"],
            )
            entry.created_at = "2026-08-29T12:34:56+08:00"
            entry.access_count = 4
            memory_store.store(entry)
            handler = _make_handler(
                app_state=SimpleNamespace(memory_store=memory_store)
            )
            handler.headers = {}
            for attr in ["send_response", "send_header", "end_headers", "wfile"]:
                setattr(handler, attr, MagicMock())
    ...
    def test_handle_memory_entries_uses_bounded_read_snapshot(self):
        import tempfile

        with tempfile.TemporaryDirectory() as tmp:
            memory_store = _main.MemoryStore(memory_dir=tmp)
            small = _main.MemoryEntry.create(
                _main.MemoryType.USER,
                "bounded-small",
                "small body",
            )
            large = _main.MemoryEntry.create(
                _main.MemoryType.USER,
                "bounded-large",
                "large body",
            )
            memory_store.store(small)
            large_path = Path(memory_store.store(large))
            large_path.write_bytes(large_path.read_bytes() + b"x" * 2048)
            handler = _make_handler(
                app_state=SimpleNamespace(memory_store=memory_store)
            )
            handler.headers = {}
        ...
        self.assertEqual(
            [item["title"] for item in parsed["entries"]],
            [small.title],
        )
    ...
    def test_memory_store_rejects_non_string_tags_with_stable_bad_request(self):
        import tempfile
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            handler = _make_handler(
                ...
                app_state=SimpleNamespace(
                    ...
                    increment_requests=lambda: None,
                ),
            )
            handler.headers = {"Content-Length": str(len(body_data))}
            handler.rfile = io.BytesIO(body_data)
            ...
            parsed = json.loads(handler.wfile.write.call_args[0][0])
            stored_files = [
                path for path in Path(tmp).glob("*.md") if path.name != "MEMORY.md"
            ]
    ...
    def test_handle_events_serializes_its_app_state_event_bus(self):
        event_bus = EventBus()
        event_bus.publish(
            Event(
        ...
        handler = _make_handler(app_state=SimpleNamespace(event_bus=event_bus))
        for attr in ["send_response", "send_header", "end_headers", "wfile"]:
            setattr(handler, attr, MagicMock())
...
class TestMainHTTPStateLifecycle(unittest.TestCase):
    def test_default_worker_receives_injected_registry_role_tool_assembly(self):
        from core.contracts.role_tool_catalog import RoleToolAssembly

        supervisor = MagicMock()
        registry = MagicMock()
        assembly = RoleToolAssembly.for_catalog()
        registry.role_tool_assembly.return_value = assembly
        with patch.object(
            _main,
        ...
        try:
            ...
            self.assertEqual(
                runner_config["role_tool_assembly"],
                assembly.to_dict(),
            )
            registry.role_tool_assembly.assert_called_once_with()
        finally:
            app_state.shutdown()

    def test_injected_role_supervisor_does_not_request_unused_assembly(self):
        supervisor = MagicMock()
        registry = MagicMock()

        app_state = _main.AppState(
            terminal=MagicMock(),
            role_tasks=supervisor,
            capability_registry=registry,
        )
        try:
            self.assertIs(app_state.role_tasks, supervisor)
            registry.role_tool_assembly.assert_not_called()
        finally:
            app_state.shutdown()

    def test_app_state_uses_injected_plugin_manager_and_event_bus(self):
        event_bus = EventBus()
        manager = MagicMock()
        ...
        try:
            self.assertIs(app_state.event_bus, event_bus)
            self.assertIs(app_state.plugin_manager, manager)
        finally:
            app_state.shutdown()
    ...
    def test_shutdown_closes_plugins_and_destroys_events_after_failure(self):
        order = []
        role_tasks = _FakeRoleTasks(order, failures=1)
        role_dispatch = _FakeRoleDispatch(role_tasks)
        terminal = MagicMock()
        terminal.close.side_effect = lambda: order.append("terminal")
        event_bus = EventBus()
        event_bus.destroy = lambda: order.append("event_bus")
        manager = MagicMock()
        manager.event_bus = event_bus
