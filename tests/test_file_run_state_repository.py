...
from adapters import file_run_state_repository as run_state_module
...
class TestFileRunStateRepository(unittest.TestCase):
    ...
    def test_recovery_directory_scan_has_a_fixed_entry_budget(self):
        self.assertEqual(run_state_module._MAX_RECOVERY_DIRECTORY_ENTRIES, 8_192)

    def test_recovery_resume_rejects_over_budget_revision_children(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            run_directory = root / "runs" / "run-20260716-043000"
            revisions = run_directory / "revisions"
            revisions.mkdir(parents=True)
            (revisions / "1").mkdir()
            (revisions / ".1.first.staging").mkdir()
            (revisions / ".1.second.staging").mkdir()

            with patch.object(
                run_state_module,
                "_MAX_RECOVERY_DIRECTORY_ENTRIES",
                2,
            ):
                self.assertFalse(
                    FileRunStateRepository._can_resume_unpublished_revision(
                        run_directory,
                        1,
                    )
                )

    def test_recovery_directory_scan_stops_at_first_over_budget_entry(self):
        calls = []
        ...
        class _GuardedScanner:
            ...
            def __next__(self):
                ...
                return SimpleNamespace(path=f"entry-{len(calls)}")
        ...
        with patch.object(
            ...
            with self.assertRaisesRegex(
                RunStateIntegrityError,
                "too many recovery directory entries",
            ):
                FileRunStateRepository._bounded_directory_children(
                    Path("recovery"),
                    2,
                )
    ...
    def test_cleanup_directory_scan_stops_at_first_over_budget_entry(self):
        calls = []
        ...
        class _GuardedScanner:
            ...
            def __next__(self):
                ...
                if len(calls) > 3:
                    raise AssertionError("scanner advanced beyond first overflow")
                return SimpleNamespace(name=f"entry-{len(calls)}")
        ...
        with patch.object(
            ...
            with self.assertRaisesRegex(
                RunStateIntegrityError,
                "too many cleanup directory entries",
            ):
                FileRunStateRepository._bounded_fd_directory_names(10, 2)
    ...
    def test_recovery_reads_are_bounded_and_snapshot_total_is_enforced(self):
        self.assertTrue(hasattr(run_state_module, "_read_bounded_bytes"))
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "recovery.bin"
            limit = run_state_module._MAX_RECOVERY_FILE_BYTES
            ...
            class _ReadGuard:
                ...
                def __exit__(self, *args):
                    self._handle.close()
                    if args[0] is None and self._reads != 1:
                        raise AssertionError("recovery content must be read once")
                    return False

                def read(self, size=-1):
                    self._reads += 1
                    if self._reads != 1:
                        raise AssertionError("recovery content must not be chunked")
                    if size != limit + 1:
                        raise AssertionError("recovery read must include one sentinel")
            ...
            def guarded_open(path, mode="r", *args, **kwargs):
                ...
                if mode != "rb":
                    handle.close()
                    raise AssertionError("recovery reads must use binary mode")
                return _ReadGuard(handle)

            with patch.object(Path, "open", guarded_open):
                content = run_state_module._read_bounded_bytes(
                    target,
                    label="fixture",
                )
            ...
            with patch.object(
                ...
                with self.assertRaises(RunStateIntegrityError) as raised:
                    run_state_module._read_bounded_bytes(target, label="fixture")
            self.assertIn("size", str(raised.exception))

        with tempfile.TemporaryDirectory() as tmp:
            repo = self.make_repo(tmp)
            repo.save(self.make_state(), "schema_version: 1\n")
            with patch.object(run_state_module, "_MAX_RECOVERY_TOTAL_BYTES", 1):
                with self.assertRaisesRegex(RunStateIntegrityError, "size"):
                    repo.load_active()
    ...
    def test_save_flushes_every_touched_directory(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ...
            self.assertIn(root / "runs", flushed)
            self.assertIn(root / "runs" / state.run_id, flushed)
            self.assertIn(revisions, flushed)
            self.assertTrue(
                any(entry.name.endswith(".staging") for entry in flushed),
                flushed,
            )

    def test_archive_flushes_the_directories_it_mutates(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            repo = self.make_repo(root)
            state = self.make_state()
            repo.save(state, "schema_version: 1\n")
            run_directory = root / "runs" / state.run_id
            flushed = []
            ...
            self.assertIn(run_directory, flushed)
            self.assertIn(root, flushed)
            self.assertIsNone(repo.load_active())

    def test_directory_flush_tolerates_unsupported_filesystems(self):
        with tempfile.TemporaryDirectory() as tmp:
            with patch.object(
                durable_directory,
                ...
                run_state_module._fsync_directory(Path(tmp))

    def test_directory_flush_fails_closed_on_io_errors(self):
        with tempfile.TemporaryDirectory() as tmp:
            closed = []
    ...
    @unittest.skipUnless(os.name == "nt", "Windows has no directory fsync")
    def test_windows_directory_flush_is_disabled(self):
        with tempfile.TemporaryDirectory() as tmp:
            self.assertFalse(durable_directory.supports_directory_fsync())
    ...
    def test_posix_save_flushes_real_directories(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            repo = self.make_repo(root)
            state = self.make_state()

            self.assertTrue(durable_directory.supports_directory_fsync())
            stored = repo.save(state, "schema_version: 1\n")

            self.assertEqual(stored.state, state)
            run_state_module._fsync_directory(root)
    ...
    def test_posix_revision_cleanup_does_not_follow_swapped_directory(self):
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as outside:
            ...
            def swap_before_scanning(directory):
                nonlocal swapped
            ...
            with patch(
                "adapters.file_run_state_repository.os.scandir",
                side_effect=swap_before_scanning,
            ):
    ...
    def test_cleanup_rechecks_the_opened_revision_directory_identity(self):
        ...
        with (
            ...
            patch(
                "adapters.file_run_state_repository.FileRunStateRepository._bounded_fd_directory_names",
    ...
    def test_revision_cleanup_overflow_does_not_prune_any_snapshot(self):
        repo = self.make_repo(Path("repository"))
        discard = Mock()
        ...
        with (
            ...
            patch.object(
                FileRunStateRepository,
                "_bounded_fd_directory_names",
                side_effect=RunStateIntegrityError(
                    "too many cleanup directory entries"
                ),
            ) as bounded_names,
            patch.object(repo, "_discard_flat_directory", discard),
        ):
            repo._prune_revision_snapshots("run-20260716-043000", 2)

        bounded_names.assert_called_once_with(
            10,
            run_state_module._MAX_RECOVERY_DIRECTORY_ENTRIES,
        )
        discard.assert_not_called()

    def test_flat_directory_cleanup_overflow_does_not_unlink_any_child(self):
        repo = self.make_repo(Path("repository"))
        ...
        with (
            ...
            patch.object(repo, "_directory_entry_matches_fd", return_value=True),
            patch(
                "adapters.file_run_state_repository.os.stat",
                return_value=Mock(st_mode=0o100644),
            ),
            patch("adapters.file_run_state_repository.os.close"),
            patch("adapters.file_run_state_repository.os.rmdir") as rmdir,
            patch("adapters.file_run_state_repository.os.unlink") as unlink,
        ...
        bounded_names.assert_called_once_with(20, len(run_state_module._FILE_NAMES))
        unlink.assert_not_called()
        rmdir.assert_not_called()
    ...
    def test_committed_manifest_with_failed_read_back_is_idempotently_retried(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            real_bounded_read = run_state_module._read_bounded_bytes

            def fail_active_manifest_read_back(path, *, label):
                if path == repo._active_path and label.endswith("read-back"):
                    raise OSError("active manifest read-back failed")
                return real_bounded_read(path, label=label)
            ...
            with patch.object(
                run_state_module,
                "_read_bounded_bytes",
                side_effect=fail_active_manifest_read_back,
            ):
