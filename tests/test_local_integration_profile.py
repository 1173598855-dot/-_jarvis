...
class TestLocalIntegrationProfile(unittest.TestCase):
    ...
    def test_request_json_rejects_response_body_over_limit(self):
        class Response:
            status = 200
            ...
            def read(self, size=-1):
                return b'"12345"' if size < 0 else b'"12345"'[:size]

        with patch.object(
            self.profile, "MAX_PROFILE_RESPONSE_BYTES", 4, create=True
        ), patch.object(
            self.profile.urllib.request, "urlopen", return_value=Response()
        ):
    ...
    def test_bounded_response_reader_is_used_for_sse_payloads(self):
        class Response:
            def read(self, size=-1):
                return b"12345" if size < 0 else b"12345"[:size]
        ...
        with patch.object(
            ...
            with self.assertRaises(self.profile.ProfileFailure) as raised:
                self.profile._read_bounded_response(
                    Response(), "http://127.0.0.1/api/ollama/chat/stream"
                )
