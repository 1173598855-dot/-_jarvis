...
from adapters import git_workspace as git_workspace_module
...
class TestGitWorkspaceInspector(unittest.TestCase):
    ...
    def test_snapshot_uses_only_fixed_commands_and_sanitized_environment(self):
        ...
        outputs = (
            FakeProcess(b"a" * 40 + b"\n"),
            FakeProcess(b"codex/test\n"),
            FakeProcess(b" M src/main.py\0"),
        )
        with patch(
            "adapters.git_workspace.subprocess.Popen",
            side_effect=outputs,
        ) as run:
    ...
    def test_snapshot_rejects_git_output_overflow_before_parsing(self):
        class OverflowingProcess:
            def __init__(self, overflowing_stream):
                output = b"x" * 33
                self.stdout = io.BytesIO(output if overflowing_stream == "stdout" else b"")
                self.stderr = io.BytesIO(output if overflowing_stream == "stderr" else b"")
                self.returncode = 0
        ...
        for overflowing_stream in ("stdout", "stderr"):
            with self.subTest(stream=overflowing_stream):
                with (
                    patch.object(git_workspace_module, "_MAX_GIT_OUTPUT_BYTES", 32),
                    patch.object(
                        git_workspace_module.subprocess,
                        "Popen",
                        return_value=OverflowingProcess(overflowing_stream),
                    ),
                ):
                    with self.assertRaisesRegex(ValueError, "git output exceeds"):
                        GitWorkspaceInspector(Path.cwd()).snapshot()
