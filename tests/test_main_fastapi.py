...
@contextmanager
def _isolated_memory_client():
    """Serve the real routes over a temporary memory directory.

    Memory write regressions must never store entries in the repository's
    live `.auto-memory` directory, so each writer owns its own state and
    lets the lifespan shut it down before the directory is removed.
    """
    ...
    with tempfile.TemporaryDirectory(
        prefix=".test-fastapi-memory-",
    ) as memory_dir:
        isolated_state = main_fastapi.AppState(memory_dir=memory_dir)
        with TestClient(
            main_fastapi.create_app(isolated_state),
...
class TestMainFastapiSyntax(unittest.TestCase):
    ...
    def test_isolated_memory_entries_preserve_v2_fields_and_clean_content(self):
        from fastapi.testclient import TestClient
        ...
        with tempfile.TemporaryDirectory() as memory_dir:
            isolated_state = main_fastapi.AppState(memory_dir=memory_dir)
            entry = main_fastapi.MemoryEntry.create(
                main_fastapi.MemoryType.REFERENCE,
                "fastapi round trip",
                "clean fastapi body",
                tags=["api", "fastapi"],
            )
            entry.created_at = "2026-08-29T13:45:00+08:00"
            entry.access_count = 5
            isolated_state.memory_store.store(entry)
            isolated_app = main_fastapi.create_app(isolated_state)
    ...
    def test_isolated_memory_entries_use_bounded_read_snapshot(self):
        from fastapi.testclient import TestClient
        ...
        with tempfile.TemporaryDirectory() as memory_dir:
            isolated_state = main_fastapi.AppState(memory_dir=memory_dir)
            small = main_fastapi.MemoryEntry.create(
                main_fastapi.MemoryType.USER,
                "bounded-small",
                "small body",
            )
            large = main_fastapi.MemoryEntry.create(
                main_fastapi.MemoryType.USER,
                "bounded-large",
                "large body",
            )
            isolated_state.memory_store.store(small)
            large_path = Path(isolated_state.memory_store.store(large))
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            [item["title"] for item in response.json()["entries"]],
            [small.title],
        )

    def test_memory_store_rejects_non_string_tags_before_storage(self):
        from fastapi.testclient import TestClient
        ...
        with tempfile.TemporaryDirectory() as memory_dir:
            ...
            stored_files = [
                path for path in Path(memory_dir).glob("*.md")
...
class TestPluginLifecycleEndpoints(unittest.TestCase):
    ...
    def test_plugin_and_event_routes_use_the_injected_app_state(self):
        from fastapi.testclient import TestClient
        ...
        self.assertEqual(plugins.json(), {"plugins": []})
    ...
    def test_shutdown_closes_plugins_and_destroys_events_after_task_failure(self):
        import main_fastapi
        ...
        role_tasks.shutdown = shutdown_role_tasks
        role_dispatch = _FakeRoleDispatch(results={}, supervisor=role_tasks)
        event_bus = EventBus()
...
class TestPydanticModels(unittest.TestCase):
    ...
    def test_plugin_load_request_forbids_worker_control_fields(self):
        from pydantic import ValidationError

        with self.assertRaises(ValidationError):
            self.mf.PluginLoadRequest(
                plugin_id="event-logger", runtime="python_worker"
            )
...
class TestFastAPIEndpoints(unittest.TestCase):
    ...
    def test_memory_store_returns_success(self):
        """POST /api/memory/store creates a new entry in isolated storage"""
        with _isolated_memory_client() as client:
