...
from core.contracts.role_tool_protocol import RoleToolProtocolError
...
class TestRoleWorkerSupervisor(unittest.TestCase):
    ...
    def test_receive_event_requests_a_bounded_transport_frame(self):
        class Connection:
            def __init__(self):
                self.maxlength = None
        ...
        runtime = Mock(connection=connection)

        supervisor._receive_event("task-missing", runtime)

        self.assertEqual(
            connection.maxlength,
            128 + getattr(role_worker_module, "MAX_WORKER_EVENT_OVERHEAD_BYTES", 0) + 1,
        )

    def test_receive_event_discards_the_overflow_sentinel(self):
        class Connection:
            def recv_bytes(self, maxlength=None):
                return b"x" * maxlength

        supervisor = self.make_supervisor(worker_fixtures.succeed)
        runtime = Mock(connection=Connection())
        with patch.object(supervisor, "_accept_event") as accept_event:
            supervisor._receive_event("task-missing", runtime)

        accept_event.assert_not_called()
    ...
    def test_output_normalization_stops_before_unbounded_json_dumps(self):
        class _GuardedEncoder:
            def __init__(self, *args, **kwargs):
                pass

            def iterencode(self, _value):
                yield 'x' * 32
                raise AssertionError('bounded output must stop at the first oversized chunk')

        with patch.object(role_worker_module.json, 'JSONEncoder', _GuardedEncoder):
            with patch.object(
                role_worker_module.json,
                'dumps',
                side_effect=AssertionError('bounded output must not call json.dumps'),
            ):
                with self.assertRaises(role_worker_module._WorkerOutputLimitError):
                    role_worker_module._bounded_json_value({'value': 'ignored'}, 8)
        ...
        try:
            supervisor = self.make_supervisor(
                ...
                runner_config={
                    ...
                    "role_tool_assembly": RoleToolAssembly.for_catalog().to_dict(),
                },
    ...
    def test_fixed_role_runner_uses_direct_execution_without_transport_timeout(self):
        ...
        invocation = Mock()
        standalone_secret = "sk-" + "proj-abcdefghijklmnopqrstuvwxyz0123456789"
    ...
    def test_fixed_role_runner_rejects_invalid_assembly_before_dependencies(self):
        request = WorkerTaskRequest.new("engineer", "inspect", 10)
        valid_config = {
            "ollama_base_url": "http://fixture.local",
            "role_model": "fixture-model",
            "memory_dir": "C:/trusted-memory",
        ...
        invalid_evidence = (
            None,
            {
                **RoleToolAssembly.for_catalog().to_dict(),
                "catalog_sha256": "0" * 64,
            },
        )
        ...
        for evidence in invalid_evidence:
            with self.subTest(evidence=evidence):
                ...
                if evidence is not None:
                    config["role_tool_assembly"] = evidence
                with (
                    patch("core.kernel.ollama_manager.OllamaManager") as manager_type,
                    patch("core.brain.agent_factory.AgentFactory") as factory_type,
                    self.assertRaises(RoleToolProtocolError),
                ):
                    execute_role_task(request.to_dict(), config)

                manager_type.assert_not_called()
                factory_type.assert_not_called()
