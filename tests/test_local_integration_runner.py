...
class TestLocalIntegrationRunner(unittest.TestCase):
    ...
    def test_fixture_rejects_oversized_declared_body_before_reading(self):
        module = _load_fixture_module()

        def unexpected_read(_size):
            raise AssertionError("oversized fixture body should be rejected first")

        request = SimpleNamespace(
            headers={"Content-Length": "33"},
            rfile=SimpleNamespace(read=unexpected_read),
        )
        with patch.object(module, "MAX_FIXTURE_REQUEST_BYTES", 32):
            with self.assertRaisesRegex(ValueError, "fixture request body exceeds"):
                module._read_bounded_json_body(request)
