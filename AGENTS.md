...
- `src/`：35 个 Python 文件、10,705 行非空代码/文档行；服务端源代码统一使用 Python。
- `frontend/src/`：38 个 TypeScript/TSX 文件。
- `tests/`：56 个 `test_*.py` 文件；规范聚合套件 442 个用例；完整 discovery 1297 个用例。
...
Stage D 能力注册表已启动：当前只读扫描可生成 19 个 Skill、2 个 Plugin 和 1 个直接导出 UI 组件的版本化记录；兼容解析、安全包存储、可逆生命周期和 API/UI 接入仍在后续迭代中。
...
Current verified test baseline for Iteration 135: aggregate 442 total (440 passed, 2 skipped); full discovery 1297 total (1295 passed, 2 skipped); Vitest 129 passed; Playwright 5 passed and 1 conditional skip.
