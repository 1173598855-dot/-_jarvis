  ...
  apiMocks.capabilityRegistry.mockResolvedValue({
    ...
    capabilities: [
      ...
      },
      {
        ...
        version: null,
        description: 'Read bounded Git metadata from the trusted repository',
...
describe('PluginsView', () => {
  it('renders registry counts and trust metadata without controls on read-only kinds', async () => {
    ...
    expect(screen.getByText('1 角色工具')).not.toBeNull();
    ...
    const roleTool = screen.getByRole('listitem', { name: '能力 Repository Metadata' });
    expect(within(roleTool).getByText('角色工具')).not.toBeNull();
    expect(within(roleTool).getByText('repository.read')).not.toBeNull();
    expect(within(roleTool).queryByRole('button')).toBeNull();
  });
