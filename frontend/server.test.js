...
let ollamaChatMode = 'healthy';
let ollamaOverflowBytesSent = 0;
    ...
    if (req.method === 'GET' && req.url === '/api/tags') {
      if (ollamaStatusMode === 'oversized') {
        ...
        const writeChunk = () => {
          ...
          const next = chunk.subarray(0, Math.min(chunk.length, target - sent));
          sent += next.length;
          ollamaOverflowBytesSent = sent;
          res.write(next);
...
describe('API fallback contract', () => {
  test('accepts an API raw query at the 32 KiB boundary', async () => {
    const query = 'x'.repeat(32 * 1024);
    const response = await fetch(`http://127.0.0.1:${apiPort}/api/health?${query}`);

    expect(response.status).toBe(200);
  });

  test('rejects an API raw query over the 32 KiB boundary', async () => {
    const query = 'x'.repeat(32 * 1024 + 1);
    const response = await fetch(`http://127.0.0.1:${apiPort}/api/health?${query}`);
...
describe('Ollama streaming chat proxy', () => {
  ...
  test('bounds oversized Ollama models responses before the fixture finishes', async () => {
    ollamaStatusMode = 'oversized';
    ollamaOverflowBytesSent = 0;
    try {
      const response = await fetch(`http://127.0.0.1:${apiPort}/api/ollama/models`);
      expect(response.status).toBe(502);
      await expect(response.json()).resolves.toEqual({
        error: {
          ...
          message: 'Ollama models response is invalid',
        },
      });
  ...
  test('bounds oversized Ollama status responses before JSON parsing', async () => {
    ollamaStatusMode = 'oversized';
    ollamaOverflowBytesSent = 0;
    try {
      const response = await fetch(`http://127.0.0.1:${apiPort}/api/ollama/status`);
      ...
      await expect(response.json()).resolves.toEqual({
        error: {
          code: 'OLLAMA_INVALID_RESPONSE',
          message: 'Ollama status response is invalid',
        },
      });
      expect(ollamaOverflowBytesSent).toBeLessThan(16 * 1024 * 1024);
    } finally {
  ...
  test('bounds oversized non-streaming Ollama chat responses', async () => {
    ollamaChatMode = 'oversized';
    ollamaOverflowBytesSent = 0;
    try {
      const response = await fetch(`http://127.0.0.1:${apiPort}/api/ollama/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  test('bounds oversized streaming Ollama responses before line parsing', async () => {
    ollamaChatMode = 'oversized';
    ollamaOverflowBytesSent = 0;
    try {
      const response = await fetch(`http://127.0.0.1:${apiPort}/api/ollama/chat/stream`, {
...
describe('Shared API errors', () => {
  test('rejects and kills a Git child at the first byte beyond 8 MiB', async () => {
    const previousNoListen = process.env.JARVIS_TEST_NO_LISTEN;
    let runGitCommand;
    try {
      process.env.JARVIS_TEST_NO_LISTEN = '1';
      vi.resetModules();
      ({ runGitCommand } = await import('./server.js'));
      ...
      const command = runGitCommand(['log'], {
        command: 'fixture-git',
        cwd: process.cwd(),
        spawnImpl,
      });

      child.stdout.emit('data', Buffer.alloc(8 * 1024 * 1024, 0x78));
      child.stdout.emit('data', Buffer.of(0x78));
  ...
  test('bounds Git stderr independently from stdout', async () => {
    const previousNoListen = process.env.JARVIS_TEST_NO_LISTEN;
    let runGitCommand;
    try {
      process.env.JARVIS_TEST_NO_LISTEN = '1';
      ...
      child.kill = vi.fn();
      const spawnImpl = vi.fn(() => child);
      const command = runGitCommand(['status'], { spawnImpl });

      child.stdout.emit('data', Buffer.from('ok'));
      child.stderr.emit('data', Buffer.alloc(8 * 1024 * 1024, 0x65));
      child.stderr.emit('data', Buffer.of(0x65));
      child.emit('close', 1);
...
describe('Core API bridge', () => {
  ...
  test('keeps synchronous role requests alive beyond the default Core timeout with fake timers', async () => {
    ...
    try {
      ...
      await new Promise((resolve) => proxyServer.listen(0, loopbackHost, resolve));
  ...
  test('assigns bounded Worker lifecycle proxy budgets to plugin routes', async () => {
    ...
    const coreRequest = vi.fn(async (path) => ({
      status: 200,
      body: path.endsWith('/load')
        ? { plugin_id: 'event-logger', name: 'Event Logger', status: 'loaded' }
    ...
    try {
      ...
      for (const [action, timeoutMs] of Object.entries(expectedTimeouts)) {
        const response = await fetch(
          `http://127.0.0.1:${proxyPort}/api/plugins/${action}`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        expect(response.status).toBe(200);
        expect(coreRequest).toHaveBeenLastCalledWith(
          `/api/plugins/${action}`,
