...
def _read_bounded_response(response, url):
    content_length = _response_content_length(response)
    if content_length is not None and content_length > MAX_PROFILE_RESPONSE_BYTES:
        raise ProfileFailure(
            f"Response from {url} exceeds {MAX_PROFILE_RESPONSE_BYTES} bytes"
        )
    body = response.read(MAX_PROFILE_RESPONSE_BYTES + 1)
    if not isinstance(body, (bytes, bytearray, memoryview)):
        raise ProfileFailure(f"Response from {url} was not a byte body")
    body = bytes(body)
    if len(body) > MAX_PROFILE_RESPONSE_BYTES:
        raise ProfileFailure(
            f"Response from {url} exceeds {MAX_PROFILE_RESPONSE_BYTES} bytes"
        )
    return body
...
def _request(url, *, method="GET", payload=None, timeout=30):
    ...
    try:
        ...
        with error:
            body = _read_bounded_response(error, url).decode(
...
def _request_json(url, *, method="GET", payload=None, timeout=30):
    with _request(url, method=method, payload=payload, timeout=timeout) as response:
        try:
            body = _read_bounded_response(response, url)
...
def parse_sse_payload(payload):
    if len(payload.encode("utf-8")) > MAX_PROFILE_RESPONSE_BYTES:
        raise ProfileFailure(
            f"SSE response exceeds {MAX_PROFILE_RESPONSE_BYTES} bytes"
        )
    frames = []
    completed = False
    saw_content = False
...
def run_profile(express_url, model=None, timeout=30):
    ...
    try:
        ...
        with _request(
            ...
            sse_payload = _read_bounded_response(
                response, f"{base_url}/api/ollama/chat/stream"
            ).decode("utf-8")
